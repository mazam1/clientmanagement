    ...
    <div class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
            <x-card padding="false">
                <div class="p-4">
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="text-sm font-medium text-text-tertiary dark:text-dark-text-tertiary uppercase">Total Revenue</p>
                            <p class="text-2xl font-bold text-text-primary dark:text-dark-text-primary mt-1">${{ number_format($stats['total_revenue'], 2) }}</p>
                        </div>
            ...
            <x-card padding="false">
                <div class="p-4">
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="text-sm font-medium text-text-tertiary dark:text-dark-text-tertiary uppercase">Unpaid Amount</p>
                            <p class="text-2xl font-bold text-accent-danger mt-1">${{ number_format($stats['unpaid_amount'], 2) }}</p>
                        </div>
                        <div class="w-10 h-10 bg-accent-danger/10 rounded-lg flex items-center justify-center">
            ...
            <x-card padding="false">
                <div class="p-4">
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="text-sm font-medium text-text-tertiary dark:text-dark-text-tertiary uppercase">Paid Invoices</p>
                            <p class="text-2xl font-bold text-text-primary dark:text-dark-text-primary mt-1">{{ $stats['paid_invoices'] }}</p>
                        </div>
            ...
            <x-card padding="false">
                <div class="p-4">
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="text-sm font-medium text-text-tertiary dark:text-dark-text-tertiary uppercase">Unpaid Invoices</p>
                            <p class="text-2xl font-bold text-accent-warning mt-1">{{ $stats['unpaid_invoices'] }}</p>
                        </div>
        ...
        <x-card>
            <form method="GET" action="{{ route('invoices.index') }}" class="space-y-4">
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <div>
                        <label for="payment_status" class="block text-sm font-medium text-text-secondary dark:text-dark-text-secondary mb-2">Payment Status</label>
                        <select
                            name="payment_status"
                            id="payment_status"
                            class="w-full px-4 py-2 rounded-lg border border-border-medium dark:border-dark-border-medium bg-bg-primary dark:bg-dark-bg-primary text-text-primary dark:text-dark-text-primary focus:outline-none focus:ring-2 focus:ring-accent-primary focus:border-transparent"
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <div class="md:col-span-3 flex items-end gap-2">
                        ...
                        @if($filters['search'] || $filters['client_id'] || $filters['payment_status'] || $filters['date_from'])
        ...
        <x-card>
            <div class="flex items-center justify-between mb-6">
                <h3 class="text-lg font-semibold text-text-primary dark:text-dark-text-primary">Invoice List</h3>
            ...
            @else
                ...
                <div class="hidden lg:block">
                    <x-table :headers="['Invoice #', 'Client', 'Issued Date', 'Amount', 'Status', 'Actions']">
                        @foreach($invoices as $invoice)
                            <tr class="hover:bg-bg-tertiary dark:hover:bg-dark-bg-tertiary transition-colors cursor-pointer" onclick="window.location='{{ route('invoices.show', $invoice) }}'">
                                <td class="px-6 py-4">
                                    <span class="font-medium text-accent-primary">{{ $invoice->invoice_number }}</span>
                                </td>
                                <td class="px-6 py-4">
                                    <div class="flex items-center">
                                        ...
                                        <span class="font-medium text-text-primary dark:text-dark-text-primary">{{ $invoice->client->name }}</span>
                                    </div>
                                </td>
                                <td class="px-6 py-4 text-text-secondary dark:text-dark-text-secondary">{{ $invoice->issued_at->format('M d, Y') }}</td>
                                <td class="px-6 py-4 text-text-secondary dark:text-dark-text-secondary">${{ number_format($invoice->total_amount, 2) }}</td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4" onclick="event.stopPropagation()">
                                    <div class="flex items-center gap-2">
                                        @can('edit-invoices')
                                            <a href="{{ route('invoices.edit', $invoice) }}" class="text-accent-primary hover:text-accent-primary/80">
                ...
                <div class="lg:hidden space-y-4">
                    @foreach($invoices as $invoice)
                        <div class="bg-bg-secondary dark:bg-dark-bg-secondary border border-border-medium dark:border-dark-border-medium rounded-lg p-4">
                            <a href="{{ route('invoices.show', $invoice) }}" class="block">
                                <div class="flex items-center gap-3 mb-3">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        ...
                                        <h3 class="text-base font-semibold text-text-primary dark:text-dark-text-primary truncate">{{ $invoice->client->name }}</h3>
                                    </div>
                                </div>
                                <div class="space-y-2 text-sm">
                                    <div class="flex items-center justify-between">
                                        ...
                                        <span class="text-lg font-bold text-text-primary dark:text-dark-text-primary">${{ number_format($invoice->total_amount, 2) }}</span>
                                    </div>
                                    <div class="flex items-center justify-between text-text-tertiary dark:text-dark-text-tertiary text-xs">
                                        <div>
                                            @if($invoice->payment_status === 'paid')
                                            ...
                                            @else
                                                <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold bg-accent-danger text-white">
                                                    Unpaid
                                                </span>
                                            @endif
                                        </div>
                            ...
                            <div class="flex gap-2 mt-3 pt-3 border-t border-border-light dark:border-dark-border-light">
                                @can('edit-invoices')
                                    <a href="{{ route('invoices.edit', $invoice) }}" class="flex-1 flex items-center justify-center px-3 py-2 bg-accent-primary text-white rounded-lg text-sm font-medium hover:bg-accent-primary/90 transition-colors min-h-[44px]">
                                ...
                                @endcan
                                @can('delete-invoices')
                                    <form method="POST" action="{{ route('invoices.destroy', $invoice) }}" class="flex-1" onsubmit="return confirm('Are you sure you want to delete this invoice?')">
                                        @csrf
                                        @method('DELETE')
